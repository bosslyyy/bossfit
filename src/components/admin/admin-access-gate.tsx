...
export function AdminAccessGate({ children }: PropsWithChildren) {
  ...
  const loadContext = async (userId: string) => {
    ...
    try {
      ...
      const message = loadError instanceof Error ? loadError.message : "No se pudo abrir el panel admin.";
      setError(message);
      setContext(null);
    } finally {
      setLoading(false);
      ...
      <div className="flex min-h-[70vh] items-center justify-center">
        <Card className="w-full max-w-xl space-y-4">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-accent">Admin</p>
