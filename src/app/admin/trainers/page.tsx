...
export default function AdminTrainersPage() {
  ...
  const handleResetCredentials = async (trainer: AdminTrainerListItem) => {
    ...
    try {
      const payload = await resetAdminUserCredentials(session.access_token, trainer.userId);
        ...
        <div className="space-y-4">
          {trainers.map((trainer) => (
            <AdminTrainerCard
              ...
              action={
                <div className="flex flex-wrap gap-2">
                  <Link href={`/admin/users/${trainer.userId}`} className={buttonVariants({ variant: "outline", className: "h-10 px-3 text-sm" })}>
