...
import { fetchCoachMessagesForMember, getCoachMemberAccessContext } from "@/lib/supabase/coach-server";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!access) {
      return NextResponse.json({ error: "No puedes escribir a este alumno." }, { status: 403, headers: noStoreHeaders });
    }
    ...
    const messages = await fetchCoachMessagesForMember(supabase, memberId);
